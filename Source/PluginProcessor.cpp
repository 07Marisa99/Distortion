...
float DistortionAudioProcessor::doDistortion(float input) {
    switch (distortionType) {
    case OVERDRIVE:
        return doOverdrive(input);
    case DEEPFRY:
        return doDeepFry(input);
    case SINE:
        return doSine(input);
    case DESTROY: 
...
float DistortionAudioProcessor::doNoisyCircuit(float input) {
    float output = input;
    Random toast;
    if (input > (1 - distortionAmountValue) || input < (-1 + distortionAmountValue)) {
        output = input * (toast.nextFloat() / 10);
    }
    return output / (1 - distortionAmountValue);;
}
